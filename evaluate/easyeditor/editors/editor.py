...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        # MEND needs full-precision trainable tensors for loss.backward();
        # 8-bit quantized weights can break autograd in edit-time optimization.
        # However, if we are loading a 4-bit model, we should not apply further quantization.
        if self.alg_name == 'MEND':
            if isinstance(self.model_name, tuple) and hasattr(self.model_name[0].config, 'quantization_config') and self.model_name[0].config.quantization_config.load_in_4bit:
                self.quantization_to_bits = None # Already 4-bit, do nothing
            else:
                self.quantization_to_bits = None # MEND default
        else:
            self.quantization_to_bits = 8
        ...
        if type(self.model_name) is str:
            ...
            if hparams.alg_name == 'QLoRA':
                ...
                if self.quantization_to_bits != None:
                    quantization_config = BitsAndBytesConfig(
                        load_in_4bit=(self.quantization_to_bits == 4),
                        load_in_8bit=(self.quantization_to_bits == 8)
                    ) 
                    model_kwargs["quantization_config"] = quantization_config
                    target_device = f"cuda:{hparams.device}" if hasattr(hparams, 'device') else "cuda:0"
        ...
        if not hparams.model_parallel and hasattr(hparams, 'device') and hparams.alg_name != 'QLoRA' and self.quantization_to_bits == None:
    ...
    def edit_requests(self,
        ...
        if 'pre_edit' in kwargs and kwargs['pre_edit'] is not None:
            ...
            for i, request in enumerate(tqdm(requests)):
                ...
                elif self.alg_name == 'RAG':
                    assert 'train_ds' in kwargs.keys(), print('RAG need train_ds(For building knowledge base)')
        ...
        def edit_func(request):
            if self.alg_name == 'IKE' or self.alg_name == 'ICE':
                edited_model, weights_copy, icl_examples = self.model, {}, self.apply_algo(
                    ...
                    train_ds=kwargs['train_ds'] if self.alg_name in ['IKE', 'RAG'] else None
                )
            else:
                edited_model, weights_copy = self.apply_algo(
                    self.model,
        ...
        if len(all_metrics) != 0:
            _mean_metrics = summary_metrics(all_metrics)

        return all_metrics, edited_model, weights_copy, _mean_metrics
    ...
    def generate_edit(
        ...
        **kwargs
        ...
        if len(all_results) != 0:
            _mean_metrics = summary_metrics(all_results)

        return all_results, edited_model, weights_copy, _mean_metrics
