...
class MendRewriteExecutor:
    ...
    def init_model(self, model, tok, params: MENDHyperParams):
        ...
        d = torch.load(params.archive, map_location='cuda:0')
        ...
        # If the inner_params are quantized (e.g. bitsandbytes 4-bit / Params4bit),
        # `requires_grad=True` is a no-op at the tensor level — gradients can't flow
        # through the packed quantized storage. Without an upstream parameter that
        # actually tracks grad, the forward graph has no grad_fn and loss.backward()
        # raises "element 0 of tensors does not require grad".
        # Enable grad on the (non-quantized) input embedding as a backward anchor.
        # The grad we accumulate on it is unused; we only need the graph to exist
        # so the backward pass flows through the inner-param modules and fires the
        # hooks that capture __x__ and __delta__.
        any_inner_requires_grad = any(
            p.requires_grad and getattr(p, "quant_state", None) is None
            for n, p in self.model.named_parameters()
            if n in params.inner_params
        )
        if not any_inner_requires_grad:
            embed_param = self.model.get_input_embeddings().weight
            if (
                getattr(embed_param, "quant_state", None) is None
                and (torch.is_floating_point(embed_param) or torch.is_complex(embed_param))
            ):
