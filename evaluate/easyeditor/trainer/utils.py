...
#     unused_params = [p for p, g in zip(parameters, grads) if g is None]
#     if unused_params:
...
def safe_backward(loss, parameters, accumulate=1, allow_unused=False):
    ...
    # Keep only valid tensors for autograd
    valid_params = [p for p in parameters if p.requires_grad]

    grads = torch.autograd.grad(
        loss,
        valid_params,
        allow_unused=True,      # must be True for LLaMA
        retain_graph=True
    )
    ...
    # Log unused params for visibility
    unused = [p for p, g in zip(valid_params, grads) if g is None]
    if unused:
        LOG.warning(
            f"Unused parameters detected (safe): {[tuple(p.shape) for p in unused]}"
        )
