...
class GradientTransform(nn.Module):
    ...
    def forward(self, u, v, param_idx=None):
        ...
        if u.shape[-1] != self.x_dim:
            if u.numel() % self.x_dim != 0:
                raise RuntimeError(
                    f"Unexpected u shape {tuple(u.shape)} for x_dim {self.x_dim}"
                )
            u = u.reshape(-1, self.x_dim)
        if v.shape[-1] != self.delta_dim:
            if v.numel() % self.delta_dim != 0:
                raise RuntimeError(
                    f"Unexpected v shape {tuple(v.shape)} for delta_dim {self.delta_dim}"
                )
            v = v.reshape(-1, self.delta_dim)
...
class MEND(EditableModel):
    def get_shape(self, p, name=None):
        ...
        if name is not None:
            mod = parent_module(self.model, name)
    ...
    def __init__(self, model, config, model_constructor, mend=None, edit_lrs=None):
        ...
        if config.shared:
            ...
            for n, p in _inner_params(
                ...
                shape_dict[self.get_shape(p, n)].append(n)
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        # model_config is optional: state_dict() emits it, but partial
        # checkpoints (e.g. only the MEND hypernetwork) won't include it.
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        ...
        inner_param_list = list(
            _inner_params(self.model.named_parameters(), self.config.inner_params)
        )

        if self.config.shared:
            param_idx = (
                lambda n, p: self.shape_dict[self.get_shape(p, n)].index(n)
                if self.config.shared
                else None
            )  # noqa: E731
            transformed_factors = {
                n: self.mend[str(tuple(self.get_shape(p, n)))](
                    p.__x__, p.__delta__, param_idx(n, p)
                )
                for n, p in inner_param_list
            }
        else:
            transformed_factors = {
        ...
        if isinstance(self.model, transformers.GPT2LMHeadModel):
            ...
            targ = "ji"
        offload_mean_grads = getattr(self.config, "offload_mean_grads", True)
        mean_grads = {}
        ...
        idx = 0
        for n, p in inner_param_list:
            x, delta = transformed_factors[n]
            g = torch.einsum(f"bi,bj->{targ}", x, delta)

            if p.grad is None:
                info_dict[f"grad/true_mag{idx}"] = 0.0
                info_dict[f"grad/true_std{idx}"] = 0.0
                info_dict[f"grad/diff{idx}"] = 0.0
                info_dict[f"grad/cos{idx}"] = 0.0
            else:
                info_dict[f"grad/true_mag{idx}"] = p.grad.norm(2).item()
                info_dict[f"grad/true_std{idx}"] = p.grad.std().item()
                info_dict[f"grad/diff{idx}"] = (p.grad - g).norm(2).item()
                info_dict[f"grad/cos{idx}"] = F.cosine_similarity(
                    p.grad.reshape(-1), g.reshape(-1), dim=0
                ).item()

            info_dict[f"grad/pseudo_mag{idx}"] = g.norm(2).item()
            info_dict[f"grad/pseudo_std{idx}"] = g.std().item()

            if offload_mean_grads:
                mean_grads[n] = g.detach().cpu()
            else:
                mean_grads[n] = g.detach()
            idx += 1
        ...
        for n, p in edited_model.named_parameters():
            if n in pset:
                if getattr(p, 'quant_state', None) is not None:
                    # Handle bitsandbytes 4-bit quantized parameters (Params4bit).
                    # Packed storage shape e.g. (out*in//2, 1) doesn't match the
                    # full-precision update shape (out, in), so we dequantize, apply
                    # the update in floating point, then re-quantize.
                    import bitsandbytes.functional as bnb_F
                    from bitsandbytes.nn import Params4bit
                    p_dequant = bnb_F.dequantize_4bit(p.data, p.quant_state)
                    upd = updates[n]
                    if upd.device != p_dequant.device:
                        upd = upd.to(p_dequant.device)
                    new_float = p_dequant + upd.to(p_dequant.dtype)
                    new_data, new_qs = bnb_F.quantize_4bit(
                        new_float,
                        blocksize=p.quant_state.blocksize,
                        compress_statistics=p.quant_state.nested,
                        quant_type=p.quant_state.quant_type,
                    )
                    new_params.append(
                        Params4bit(new_data, requires_grad=False, quant_state=new_qs, bnb_quantized=True)
                    )
                else:
                    upd = updates[n]
                    if upd.device != p.device:
                        upd = upd.to(p.device)
                    upd = upd.to(p.dtype)
                    if upd.shape != p.shape:
                        if upd.numel() == p.numel():
                            upd = upd.reshape(p.shape)
                        else:
                            raise RuntimeError(
                                f"Update shape {tuple(upd.shape)} does not match param {n} shape {tuple(p.shape)}"
                            )
