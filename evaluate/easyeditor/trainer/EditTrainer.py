...
class EditTrainer(BaseTrainer):
    ...
    def edit_step(self, batch, training: bool):
        ...
        if self.config.train_base:
            with torch.no_grad():
                ...
                original_loc_dict = self.model.loc_loss_fn(
                    self.config, original_logits, batch["loc"]["labels"]
                )
        ...
        # Log unused parameters for debugging
        # grads = torch.autograd.grad(l_total_edit, self.model.outer_parameters(), retain_graph=True, allow_unused=True)
        # if training:
        ...
        # unused_params = [p for p, g in zip(self.model.outer_parameters(), grads) if g is None]
        # if unused_params:
        #     LOG.warning(f"Unused parameters detected during backward pass: {[p.shape for p in unused_params]}")

        # Log unused parameters for debugging (training only)
        if training:
            grads = torch.autograd.grad(
                l_total_edit,
                self.model.outer_parameters(),
            ...
            unused_params = [
                p for p, g in zip(self.model.outer_parameters(), grads) if g is None
            ]

            if unused_params:
                LOG.warning(
                    f"Unused parameters detected during backward pass: {[p.shape for p in unused_params]}"
                )
    ...
    def train_step(self, batch):
        ...
        if self.global_iter > 0 and self.global_iter % self.config.accumulate_bs == 0:
            ...
            grad_ok = True
            for p in self.model.outer_parameters():
            ...
            if grad_ok:
                ...
                if torch.isnan(grad_norm) or torch.isinf(grad_norm):
                    self.opt.zero_grad()
                    info_dict["grad"] = float('nan')
                else:
                    info_dict["grad"] = grad_norm.item()
                    self.opt.step()
    ...
    #             self.model.outer_parameters(),
    #             self.config.grad_clip,
    #             error_if_nonfinite=False  # Keeping this for handling non-finite gradients
    #         )
            
    #         # Check for non-finite gradients
    #         if torch.isnan(grad).any() or torch.isinf(grad).any():
    #             print(f"NaN or Inf detected in gradients at iteration {self.global_iter}")
    #             return info_dict  # Skip this step
