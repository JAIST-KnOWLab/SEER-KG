...
def get_model(config):
    if hasattr(config, "model") and config.model is not None:
        # Use preloaded model if provided by the caller.
        return config.model
    if config.model_class == "BertClassifier":
        ...
        if getattr(config, "disable_quantization", False):
            load_kwargs.update(
                {
        ...
        try:
            ...
            # )
            model = ModelClass.from_pretrained(config.model_name, **load_kwargs)
        except (AttributeError, ModuleNotFoundError) as e:
            # Fallback: use AutoModelForCausalLM for unknown model types
            print(f"[WARNING] Failed to load {config.model_class} ({e}). Using AutoModelForCausalLM as fallback.")
