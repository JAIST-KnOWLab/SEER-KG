            ...
            if (not self.config.debug) or self.config.save:
                if self.config.model_parallel:
                    if os.path.isfile(self.save_path):
                        ...
                        # )
                        self.model.load_state_dict(archive["model"])
                    # else: no intermediate checkpoint was saved during training;
                    # proceed to final eval with the current in-memory model state.
                else:
                    ...
                    # self.model.load_state_dict(archive["model"])
                    # self.model.to(self.config.device)
                    # Only load checkpoint if archive is explicitly provided
                    if getattr(self.config, "archive", None):
                        if not os.path.isfile(self.config.archive):
                            raise ValueError(f"Archive must be a file, got: {self.config.archive}")

                        archive = torch.load(self.config.archive, map_location="cpu")
                        # LOG.info(
                        #     f"Loading best model from step {archive['step']}, elapsed time {archive['elapsed_time']}"
                        # )
                        self.model.to("cpu")
                        ...
                        self.model.to(self.config.device)
                    # else:
                        # LOG.info("No archive provided. Training from scratch.")
